...
function renderOnlineUsers() {
    ...
    ids.forEach(id => {
        ...
        userDiv.innerHTML = `${requestBadge}${bubbleHTML}<img src="${escapeHtml(user.avatarId || 'image_0.gif')}" alt="아바타" class="online-user-avatar"><div style="display:flex; flex-direction:column; width:100%; flex: 1; justify-content: flex-end;"><span class="name-text-fit ${getNameClass(displayId)}">${escapeHtml(displayId)}</span><span style="font-size:10px; color:#ff9800; font-weight:bold; min-height:12px; line-height:1; margin-bottom:2px;">${(isMe) ? '(나)' : ''}</span></div>`;
...
function renderStudentShop() {
    ...
    localShopItems.forEach(avatarId => {
        ...
        const avatarName = localShopNames[dbKey] || `${avatarId.split('.')[0]}번 아바타`; const nameText = `<span class="name-text-fit ${getNameClass(avatarName)}">${escapeHtml(avatarName)}</span>`;
