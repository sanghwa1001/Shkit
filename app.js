...
// ==========================================
// 🏹 몬스터 헌터 인게임용 전역 독립 변수 모음
// ==========================================
const HUNT_BASE_WIDTH  = 800;
const HUNT_BASE_HEIGHT = 450;

let huntWordList    = [];   // [{ en, ko }]
let huntQuizQueue   = [];
let huntCurrentQuiz = null;
let huntQuizAnswered = false;

let huntScore          = 0;
let huntCorrectCount   = 0;
let huntWrongCount     = 0;
let huntMonsters       = [];
let huntProjectiles    = [];
let huntIsAttacking    = false;
let huntIsGameStarted  = false;
let huntIsGameOver     = false;
let huntSpawnInterval  = 1000;
let huntSpawnTimer;
let huntMoveTimer;
let huntCollisionTimer;
let huntResizeObserver = null;

// 궁극기
const HUNT_ULTIMATE_MAX = 10;
let huntUltimateCharge  = 0;
let huntIsUltimateReady = false;

const HUNT_MAX_MONSTERS  = 5;
const HUNT_PLAYER_LEFT   = 50;
const HUNT_PLAYER_WIDTH  = 47;
const HUNT_MONSTER_HIT_W = 40;
const HUNT_GRAVITY       = 0.5;
const HUNT_JUMP_FORCE    = -8;
const HUNT_GROUND_Y      = 0;

const HUNT_MONSTER_TYPES = [
...
function huntRefreshGif(filename) {
    return filename + '?t=' + Date.now();
}

// ── 진입 / 퇴장 ──────────────────────────────────────────
function openHunterGamePage() {
    if (!currentSelectedData || !localLearningData[currentSelectedData]) {
        alert('학습 내용을 먼저 선택해 주세요! ✏️');
        showPage('student-select-data-page');
        return;
    ...
    const selectedSet = localLearningData[currentSelectedData];
    if (!selectedSet.words || selectedSet.words.length < 4) {
        alert('단어가 4개 이상인 학습 데이터를 선택해 주세요.');
        return;
    }
    huntWordList = selectedSet.words.map(w => ({ en: String(w.eng), ko: String(w.kor) }));

    document.querySelector('.container').classList.add('game-mode');
    huntResetEngineUI();
    showPage('hunter-page');
    huntStartResizeObserver();
}

function exitHunterGamePage() {
    if (huntIsGameStarted) huntTriggerGameOver();
    huntStopResizeObserver();
    huntCleanupAll();
    document.querySelector('.container').classList.remove('game-mode');
    showPage('student-solo-game-page');
}

// ── ResizeObserver (상티런과 동일 방식) ──────────────────
function huntUpdateScale() {
    const wrapper = document.getElementById('hunt-game-wrapper');
    const canvas  = document.getElementById('hunt-game-canvas');
    if (!wrapper || !canvas) return;

    const scale = wrapper.clientWidth / HUNT_BASE_WIDTH;
    canvas.style.setProperty('--hunt-game-scale', scale);

    // 퀴즈 패널
    const panel = document.getElementById('hunt-quiz-panel');
    const word   = document.getElementById('hunt-quiz-word');
    if (panel && word) {
        panel.style.top         = (50 * scale) + 'px';
        panel.style.width       = (340 * scale) + 'px';
        panel.style.padding     = (8 * scale) + 'px ' + (16 * scale) + 'px';
        panel.style.borderRadius = (14 * scale) + 'px';
        word.style.fontSize     = (24 * scale) + 'px';
    }

    // 보기 버튼
    const choices = document.getElementById('hunt-quiz-choices');
    const btns    = document.querySelectorAll('.hunt-quiz-btn');
    if (choices) {
        choices.style.bottom = (5  * scale) + 'px';
        choices.style.left   = (8  * scale) + 'px';
        choices.style.width  = (724 * scale) + 'px';
        choices.style.gap    = (8  * scale) + 'px';
    }
    btns.forEach(btn => {
        btn.style.fontSize     = (14 * scale) + 'px';
        btn.style.padding      = (8 * scale) + 'px ' + (4 * scale) + 'px';
        btn.style.height       = (52 * scale) + 'px';
        btn.style.borderRadius = (10 * scale) + 'px';
    });

    // 궁극기
    const ultWrap = document.getElementById('hunt-ultimate-btn-wrap');
    const ultBtn  = document.getElementById('hunt-ultimate-btn');
    const iconWrap = document.getElementById('hunt-ultimate-icon-wrap');
    if (ultWrap && ultBtn && iconWrap) {
        ultWrap.style.bottom = (5  * scale) + 'px';
        ultWrap.style.right  = (8  * scale) + 'px';
        ultWrap.style.width  = (52 * scale) + 'px';
        ultBtn.style.borderRadius = (10 * scale) + 'px';
        const iconSize = (52 * scale) + 'px';
        iconWrap.style.width  = iconSize;
        iconWrap.style.height = iconSize;
    }
}

function huntStartResizeObserver() {
    const wrapper = document.getElementById('hunt-game-wrapper');
    if (!wrapper || typeof ResizeObserver === 'undefined') return;
    huntStopResizeObserver();
    huntResizeObserver = new ResizeObserver(() => huntUpdateScale());
    huntResizeObserver.observe(wrapper);
    huntUpdateScale();
}

function huntStopResizeObserver() {
    if (huntResizeObserver) { huntResizeObserver.disconnect(); huntResizeObserver = null; }
}

// ── UI 초기화 ─────────────────────────────────────────────
function huntResetEngineUI() {
    const scoreEl = document.getElementById('hunt-score-display');
    if (scoreEl) scoreEl.textContent = '0';

    const startOv = document.getElementById('huntStartOverlay');
    const gameOv  = document.getElementById('huntGameOver');
    const resultEl = document.getElementById('hunt-result');
    if (startOv)  startOv.classList.add('show');
    if (gameOv)   gameOv.classList.remove('show');
    if (resultEl) resultEl.innerHTML = '';

    const quizPanel   = document.getElementById('hunt-quiz-panel');
    const quizChoices = document.getElementById('hunt-quiz-choices');
    if (quizPanel)   quizPanel.style.display   = 'none';
    if (quizChoices) quizChoices.style.display = 'none';

    const playerImg    = document.getElementById('hunt-player-img');
    const playerEffect = document.getElementById('hunt-player-effect');
    if (playerImg)    playerImg.src = 'huntcharacter1.gif';
    if (playerEffect) playerEffect.style.display = 'none';

    huntResetUltimateGauge();
}

// ── 퀴즈 로직 ────────────────────────────────────────────
function huntShuffleArray(arr) {
    const a = [...arr];
...
function huntBuildQuizQueue() {
    huntQuizQueue = huntShuffleArray([...Array(huntWordList.length).keys()]);
}

function huntNextQuiz() {
    if (huntWordList.length < 4) return;
    if (huntQuizQueue.length === 0) huntBuildQuizQueue();

    const idx     = huntQuizQueue.pop();
    const correct = huntWordList[idx];
    const pool    = huntWordList.filter((_, i) => i !== idx);
    const distractors = huntShuffleArray(pool).slice(0, 3).map(w => w.ko);
    const choices  = huntShuffleArray([correct.ko, ...distractors]);
    const answerIdx = choices.indexOf(correct.ko);

    huntCurrentQuiz  = { en: correct.en, ko: correct.ko, choices, answerIdx };
    huntQuizAnswered = false;

    const wordEl = document.getElementById('hunt-quiz-word');
    if (wordEl) wordEl.textContent = correct.en;

    const btns = document.querySelectorAll('.hunt-quiz-btn');
    btns.forEach((btn, i) => {
        btn.textContent = choices[i];
        btn.className   = 'hunt-quiz-btn';
        btn.disabled    = false;
    });
    ...
    const wrapper = document.getElementById('hunt-game-wrapper');
    if (wrapper) {
        const scale = wrapper.getBoundingClientRect().width / HUNT_BASE_WIDTH;
        const baseFontSize = 14 * scale;
        const minFontSize  = baseFontSize * 0.55;
        const estWidth = (760 * scale - 8 * scale * 3) / 4 - 8 * scale * 2;
        const tmpCanvas = document.createElement('canvas');
        const ctx = tmpCanvas.getContext('2d');
        btns.forEach(btn => {
            let fs = baseFontSize;
            ctx.font = `${fs}px Jua, sans-serif`;
            while (ctx.measureText(btn.textContent).width > estWidth && fs > minFontSize) {
                fs -= 0.5;
                ctx.font = `${fs}px Jua, sans-serif`;
            }
...
function huntHandleQuizAnswer(selectedIdx) {
    if (huntQuizAnswered || !huntCurrentQuiz || !huntIsGameStarted || huntIsGameOver) return;
    huntQuizAnswered = true;

    const btns = document.querySelectorAll('.hunt-quiz-btn');
    btns.forEach(btn => btn.disabled = true);

    if (selectedIdx === huntCurrentQuiz.answerIdx) {
        btns[selectedIdx].classList.add('correct');
        huntCorrectCount++;
        huntUseSkill();
        setTimeout(() => huntNextQuiz(), 300);
    } else {
        btns[selectedIdx].classList.add('wrong');
        btns[huntCurrentQuiz.answerIdx].classList.add('correct');
        huntWrongCount++;
        setTimeout(() => huntNextQuiz(), 600);
    }
}

// ── 궁극기 게이지 ─────────────────────────────────────────
function huntUpdateUltimateGauge() {
    const pct      = (huntUltimateCharge / HUNT_ULTIMATE_MAX) * 100;
    const topClip  = 100 - pct;
    const colorEl  = document.getElementById('hunt-ultimate-icon-color');
    const ultBtn   = document.getElementById('hunt-ultimate-btn');
    if (colorEl) colorEl.style.clipPath = `inset(${topClip}% 0% 0% 0%)`;
    if (huntUltimateCharge >= HUNT_ULTIMATE_MAX && !huntIsUltimateReady) {
        huntIsUltimateReady = true;
        if (ultBtn) { ultBtn.disabled = false; ultBtn.classList.add('ready'); }
    }
}

function huntChargeUltimate() {
    if (huntIsUltimateReady) return;
    huntUltimateCharge = Math.min(huntUltimateCharge + 1, HUNT_ULTIMATE_MAX);
    huntUpdateUltimateGauge();
}

function huntResetUltimateGauge() {
    huntUltimateCharge  = 0;
    huntIsUltimateReady = false;
    const ultBtn  = document.getElementById('hunt-ultimate-btn');
    const colorEl = document.getElementById('hunt-ultimate-icon-color');
    if (ultBtn)  { ultBtn.disabled = true; ultBtn.classList.remove('ready'); }
    if (colorEl) colorEl.style.clipPath = 'inset(100% 0% 0% 0%)';
}

// ── 스킬 (일반 공격) ──────────────────────────────────────
function huntUseSkill() {
    if (!huntIsGameStarted || huntIsAttacking || huntIsGameOver) return;
    huntIsAttacking = true;

    const playerImg    = document.getElementById('hunt-player-img');
    const playerEffect = document.getElementById('hunt-player-effect');
    const canvas       = document.getElementById('hunt-game-canvas');
    if (playerImg)    playerImg.src = huntRefreshGif('huntcharacter2.gif');
    if (playerEffect) { playerEffect.src = huntRefreshGif('huntskill1.gif'); playerEffect.style.display = 'block'; }

    setTimeout(() => {
        if (playerImg)    playerImg.src = 'huntcharacter1.gif';
        if (playerEffect) playerEffect.style.display = 'none';
        huntIsAttacking = false;
    }, 500);

    setTimeout(() => {
        const targeted = new Set(huntProjectiles.map(p => p.target).filter(Boolean));
        let valid = huntMonsters
            .filter(m => !m.isDead && m.pos <= 800 && m.pos >= -50 && !targeted.has(m))
            .sort((a, b) => a.pos - b.pos)
            .slice(0, 3);
        if (valid.length > 0) valid.forEach(m => huntSpawnProjectile(m));
        else huntSpawnProjectile(null);
    }, 200);
}

// ── 궁극기 ────────────────────────────────────────────────
function huntUseUltimate() {
    if (!huntIsGameStarted || huntIsGameOver || !huntIsUltimateReady || huntIsAttacking) return;
    huntIsAttacking = true;
    huntResetUltimateGauge();

    const canvas    = document.getElementById('hunt-game-canvas');
    const playerImg = document.getElementById('hunt-player-img');
    const playerEff = document.getElementById('hunt-player-effect');

    const ultScale  = (186 * 1.8) / 400;
    const ultEffect = document.createElement('img');
    ultEffect.src   = huntRefreshGif('huntskill4.gif');
    ultEffect.style.cssText = `position:absolute;bottom:137px;left:50px;z-index:50;transform:translate(-77px,127px) scale(${ultScale});transform-origin:left bottom;`;
    if (canvas) canvas.appendChild(ultEffect);
    if (playerImg)  playerImg.src = huntRefreshGif('huntcharacter2.gif');
    if (playerEff)  playerEff.style.display = 'none';

    setTimeout(() => huntSpawnUltimateProjectile(), 550);
    setTimeout(() => {
        if (playerImg) playerImg.src = 'huntcharacter1.gif';
...
function huntSpawnUltimateProjectile() {
    ...
    function moveBullet() {
        if (!huntIsGameStarted && !huntIsGameOver) { projImg.remove(); return; }
        x += SPEED;
        projImg.style.left = x + 'px';
        huntMonsters.forEach(m => {
            if (m.isDead || hitSet.has(m)) return;
            const hitX = m.pos + 55;
            if (x >= hitX - 20 && x <= hitX + 60) { hitSet.add(m); huntApplyUltimateKill(m); }
        });
...
function huntApplyUltimateKill(m) {
    if (m.isDead) return;
    m.effect.src = huntRefreshGif('huntskill6.gif');
    const hitX = m.pos + 55;
    m.effect.style.cssText += `;left:${hitX}px;bottom:168px;transform:translate(-50%,50%);display:block;`;
    setTimeout(() => { m.effect.style.display = 'none'; }, 500);
    m.isDead = true;
    clearTimeout(m.jumpTimer); clearTimeout(m.stopTimer);
    m.img.src = huntRefreshGif(m.type.die);
    huntUpdateScore(1);
    setTimeout(() => { m.element.style.opacity = '0'; }, 50);
    m.dieTimer = setTimeout(() => {
        m.element.remove(); m.effect.remove();
        const idx = huntMonsters.indexOf(m);
        if (idx > -1) huntMonsters.splice(idx, 1);
    }, 550);
}

// ── 발사체 ────────────────────────────────────────────────
function huntSpawnProjectile(targetMonster) {
    const canvas  = document.getElementById('hunt-game-canvas');
    if (!canvas) return;
    const projImg = document.createElement('img');
    projImg.src   = huntRefreshGif('huntskill2.gif');
    projImg.className = 'hunt-projectile';
    const startX = 190, startY = 168;
    projImg.style.left   = startX + 'px';
    projImg.style.bottom = startY + 'px';
    canvas.appendChild(projImg);
    huntProjectiles.push({ element: projImg, x: startX, y: startY, target: targetMonster, speed: 18 });
}

function huntMoveProjectiles() {
    for (let i = huntProjectiles.length - 1; i >= 0; i--) {
        const p = huntProjectiles[i];
        p.x += p.speed;
        p.element.style.left = p.x + 'px';
        if (p.target) {
            if (p.target.isDead) {
                const taken = new Set(huntProjectiles.filter((_, j) => j !== i).map(q => q.target).filter(Boolean));
                p.target = huntMonsters.filter(m => !m.isDead && m.pos >= p.x - 50 && !taken.has(m)).sort((a, b) => a.pos - b.pos)[0] || null;
                continue;
            }
            if (p.x >= p.target.pos + 55) {
                const hit = p.target;
                p.element.remove(); huntProjectiles.splice(i, 1);
                if (!hit.isDead) huntApplyHit(hit);
            }
        } else {
            if (p.x > 850) { p.element.remove(); huntProjectiles.splice(i, 1); }
        }
    }
}

function huntApplyHit(m) {
    m.effect.src = huntRefreshGif('huntskill3.gif');
    const hitX = m.pos + 55;
    m.effect.style.cssText += `;left:${hitX}px;bottom:168px;transform:translate(-50%,50%);display:block;`;
    setTimeout(() => { m.effect.style.display = 'none'; }, 500);
    m.hp--;
    if (m.hp > 0) {
        ...
        m.knockback = 19;
        m.img.src = huntRefreshGif(m.type.hit);
        setTimeout(() => { if (!m.isDead) { m.isHit = false; m.img.src = huntRefreshGif(m.type.walk); } }, 500);
    } else {
        m.isDead = true;
        clearTimeout(m.jumpTimer); clearTimeout(m.stopTimer);
        m.img.src = huntRefreshGif(m.type.die);
...
// ── 점수 ─────────────────────────────────────────────────
function huntUpdateScore(delta) {
    huntScore += delta;
    const el = document.getElementById('hunt-score-display');
    if (el) el.textContent = huntScore;
}

// ── 몬스터 스폰 ───────────────────────────────────────────
function huntSpawnMonster() {
    if (!huntIsGameStarted || huntIsGameOver) return;
    const canvas = document.getElementById('hunt-game-canvas');
    if (!canvas) return;

    const alive = huntMonsters.filter(m => !m.isDead).length;
    if (alive < HUNT_MAX_MONSTERS) {
        const wrapper = document.createElement('div');
        wrapper.className = 'hunt-character-wrapper';
        const rightmost = huntMonsters.filter(m => !m.isDead).reduce((max, m) => Math.max(max, m.pos), -Infinity);
        const spawnX = rightmost === -Infinity ? 800 : Math.max(800, rightmost + 60 + Math.random() * 60);
        wrapper.style.left    = spawnX + 'px';
        wrapper.style.bottom  = '143px';
        wrapper.style.opacity = '0';

        const typeIdx = Math.floor(Math.random() * HUNT_MONSTER_TYPES.length);
        const mType   = HUNT_MONSTER_TYPES[typeIdx];

        const mImg    = document.createElement('img');
        mImg.src      = huntRefreshGif(mType.walk);

        const mEffect = document.createElement('img');
        mEffect.style.cssText = 'display:none;position:absolute;z-index:15;';

        wrapper.appendChild(mImg);
        canvas.appendChild(wrapper);
        canvas.appendChild(mEffect);
        ...
        const m = huntMonsters[huntMonsters.length - 1];
        huntScheduleJump(m, jumpDelay);
    }
    huntSpawnInterval = Math.max(400, huntSpawnInterval * 0.95);
    huntSpawnTimer = setTimeout(huntSpawnMonster, huntSpawnInterval);
}

function huntScheduleJump(monster, delay) {
    monster.jumpTimer = setTimeout(() => {
        if (!monster.isDead && !monster.isHit && !monster.isStopped && huntIsGameStarted && !huntIsGameOver) huntDoJump(monster);
    }, delay);
}

function huntDoJump(monster) {
    if (monster.isJumping || monster.isDead) return;
    monster.isJumping = true;
    monster.velY = HUNT_JUMP_FORCE;
}

// ── 게임 루프 ────────────────────────────────────────────
function huntMoveLoop() {
    for (let i = huntMonsters.length - 1; i >= 0; i--) {
        const m = huntMonsters[i];
        if (!m.isDead && !m.isHit) {
            if (!m.isStopped && !m.isJumping && m.pos <= m.stopTarget) {
                m.isStopped = true;
                m.img.src = huntRefreshGif(m.type.idle);
                const dur = 1000 + Math.random() * 500;
                m.stopTimer = setTimeout(() => {
                    if (!m.isDead && !m.isHit) {
                        m.isStopped = false;
                        m.stopTarget = Math.random() < 0.75 ? m.pos - (80 + Math.random() * 200) : -9999;
                        m.img.src = huntRefreshGif(m.type.walk);
                    }
                }, dur);
            }
            if (!m.isStopped) { m.pos -= 1; m.element.style.left = m.pos + 'px'; }
        } else if (!m.isDead && m.isHit && m.knockback > 0) {
            const step = Math.min(m.knockback, 4);
            m.pos += step; m.knockback -= step; m.element.style.left = m.pos + 'px';
        }
        if (!m.isDead && m.isJumping) {
            m.velY += HUNT_GRAVITY; m.offsetY -= m.velY;
            if (m.offsetY <= HUNT_GROUND_Y) {
                m.offsetY = 0; m.velY = 0; m.isJumping = false;
                huntScheduleJump(m, 1500 + Math.random() * 3000);
            }
            m.element.style.bottom = (143 + m.offsetY) + 'px';
        }
        if (m.pos < -150) {
            clearTimeout(m.jumpTimer); clearTimeout(m.stopTimer); clearTimeout(m.dieTimer);
            m.element.remove(); m.effect.remove(); huntMonsters.splice(i, 1);
        }
    }
    huntMoveProjectiles();
    huntMoveTimer = requestAnimationFrame(huntMoveLoop);
}

function huntCheckCollision() {
    clearTimeout(huntCollisionTimer);
    if (!huntIsGameStarted || huntIsGameOver) return;
    const playerLeft  = HUNT_PLAYER_LEFT + (150 - HUNT_PLAYER_WIDTH) / 2;
    const playerRight = playerLeft + HUNT_PLAYER_WIDTH;
    for (const m of huntMonsters) {
        if (m.isDead || m.isHit) continue;
        const mCenter = m.pos + 75;
        const mLeft   = mCenter - HUNT_MONSTER_HIT_W / 2;
        const mRight  = mCenter + HUNT_MONSTER_HIT_W / 2;
        if (mLeft < playerRight && mRight > playerLeft) { huntTriggerGameOver(); return; }
    }
    huntCollisionTimer = setTimeout(huntCheckCollision, 50);
}
...
function startHunterGame() {
    const startOv = document.getElementById('huntStartOverlay');
    const gameOv  = document.getElementById('huntGameOver');
    if (startOv) startOv.classList.remove('show');
    if (gameOv)  gameOv.classList.remove('show');

    huntScore = 0; huntCorrectCount = 0; huntWrongCount = 0;
    huntIsGameStarted = true; huntIsGameOver = false;
    huntIsAttacking = false; huntSpawnInterval = 1000;

    const scoreEl = document.getElementById('hunt-score-display');
    if (scoreEl) scoreEl.textContent = '0';
    huntResetUltimateGauge();

    if (huntWordList.length >= 4) {
        huntBuildQuizQueue();
        const panel   = document.getElementById('hunt-quiz-panel');
        const choices = document.getElementById('hunt-quiz-choices');
    ...
    huntSpawnTimer    = setTimeout(huntSpawnMonster, huntSpawnInterval);
    huntMoveTimer     = requestAnimationFrame(huntMoveLoop);
    huntCollisionTimer = setTimeout(huntCheckCollision, 500);
}

function huntTriggerGameOver() {
    if (huntIsGameOver) return;
    huntIsGameOver = true; huntIsGameStarted = false;
    cancelAnimationFrame(huntMoveTimer);
    clearTimeout(huntSpawnTimer); clearTimeout(huntCollisionTimer);
    huntMonsters.forEach(m => { clearTimeout(m.jumpTimer); clearTimeout(m.stopTimer); clearTimeout(m.dieTimer); });
    ...
    const resultEl = document.getElementById('hunt-result');
    if (resultEl) resultEl.innerHTML = `🏆 점수 : ${huntScore} 점<br>⭕ 정답 : ${huntCorrectCount} 개<br>❌ 오답 : ${huntWrongCount} 개`;

    const gameOv = document.getElementById('huntGameOver');
    if (gameOv) gameOv.classList.add('show');
}

function huntCleanupAll() {
    clearTimeout(huntSpawnTimer); clearTimeout(huntCollisionTimer);
    cancelAnimationFrame(huntMoveTimer);
    huntMonsters.forEach(m => {
        clearTimeout(m.jumpTimer); clearTimeout(m.stopTimer); clearTimeout(m.dieTimer);
    ...
    huntMonsters = [];
    huntProjectiles.forEach(p => p.element.remove());
    huntProjectiles = [];
    huntIsGameStarted = false; huntIsGameOver = false; huntIsAttacking = false;
    huntSpawnInterval = 1000; huntScore = 0; huntCorrectCount = 0; huntWrongCount = 0;
    huntResetUltimateGauge();
}

// ── 이벤트 리스너 ─────────────────────────────────────────
document.addEventListener('click', (e) => {
    const btn = e.target.closest('.hunt-quiz-btn');
    if (btn) { e.stopPropagation(); huntHandleQuizAnswer(parseInt(btn.dataset.idx)); }
});

document.addEventListener('click', (e) => {
    if (e.target.closest('#hunt-ultimate-btn')) {
        e.stopPropagation();
        huntUseUltimate();
    }
});
...
window.addEventListener('resize', () => {
    if (document.getElementById('hunter-page') && document.getElementById('hunter-page').classList.contains('active')) huntUpdateScale();
});
