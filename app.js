...
function resetSangtiRunEngineUI() {
    ...
    // 🛠️ 2번 수정 반영: 페이지 진입 시 시작 대기 오버레이를 보여주고, 종료 오버레이/결과는 숨김
...
function endSangtiRunGame(){
    ...
    // 🛠️ 2번 수정 반영: 결과 내용은 미리 채워두되, '게임결과' 버튼을 눌러야 보이도록 숨김 처리
    const resultEl = document.getElementById("result");
    resultEl.innerHTML = `🏆 점수 : ${runScore} 점<br><br>⭕ 정답 : ${runCorrectCount} 개<br><br>❌ 오답 : ${runWrongCount} 개`;
    resultEl.style.display = "none";

    const showResultBtn = document.getElementById("showResultBtn");
    showResultBtn.className = "btn-orange"; showResultBtn.disabled = false; showResultBtn.innerText = "📊 게임결과";

    document.getElementById("gameOver").classList.add("show");
}

// 🛠️ 2번 수정 반영: 게임종료 오버레이에서 '게임결과' 버튼을 눌렀을 때만 점수/정답/오답 정보를 펼쳐 보여준다.
