...
//  몬스터 스폰
// =====================================================
function mhRefreshGif(filename) {
...
function mhSpawnMonster() {
    ...
    if (alive < MH_MAX_MONSTERS) {
        ...
        mImg.src = mhRefreshGif(mType.walk);
        ...
        for (let i = mhMonsters.length - 1; i >= 0; i--) {
            ...
            if (!m.isDead && !m.isHit) {
                if (!m.isStopped && !m.isJumping && m.pos <= m.stopTarget) {
                    m.isStopped = true;
                    m.img.src = mhRefreshGif(m.type.idle);
                    ...
                    m.stopTimer = setTimeout(() => {
                        if (!m.isDead && !m.isHit) {
                            ...
                            m.stopTarget = Math.random() < 0.75
                                ...
                                : -9999;
                            m.img.src = mhRefreshGif(m.type.walk);
                        }
                    }, stopDuration);
                }
                if (!m.isStopped) {
...
function mhUseSkill() {
    ...
    mhPlayerImg.src = mhRefreshGif('huntcharacter2.gif');
    mhPlayerEffect.src = mhRefreshGif('huntskill1.gif');
...
function mhSpawnProjectile(targetMonster) {
    ...
    projImg.src = mhRefreshGif('huntskill2.gif');
...
function mhApplyHit(m) {
    m.effect.src = mhRefreshGif('huntskill3.gif');
    const hitX = m.pos + 55;
    ...
    if (m.hp > 0) {
        ...
        clearTimeout(m.stopTimer);
        m.knockback = 19;
        m.img.src   = mhRefreshGif(m.type.hit);
        setTimeout(() => {
            if (!m.isDead) { m.isHit = false; m.img.src = mhRefreshGif(m.type.walk); }
        }, 500);
    } else {
        m.isDead = true;
        clearTimeout(m.jumpTimer); clearTimeout(m.stopTimer);
        m.img.src = mhRefreshGif(m.type.die);
        mhUpdateScore(1);
        mhChargeUltimate();
...
function mhUseUltimate() {
    ...
    ultEffect.src   = mhRefreshGif('huntskill4.gif');
...
function mhSpawnUltimateProjectile() {
    const projImg = document.createElement('img');
    projImg.src = mhRefreshGif('huntskill5.gif');
    projImg.className = 'mh-projectile';
...
function mhApplyUltimateKill(m) {
    if (m.isDead) return;
    m.effect.src = mhRefreshGif('huntskill6.gif');
    const hitX = m.pos + 55;
    m.effect.style.left      = hitX + 'px';
    m.effect.style.bottom    = '168px';
    m.effect.style.transform = 'translate(-50%, 50%)';
