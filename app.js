...
function updateSangtiRunScale() {
    ...
    const btnGroup = document.getElementById("runBtnGroup");
    if (!wrapper || !gameCanvas) return;

    // 🛠️ 버튼 그룹 실측 높이를 CSS 변수로 반영 → #game-wrapper의 height가 매직넘버(80px) 대신
    // 실제 버튼 영역 크기 기준으로 계산되어, 화면 어떤 비율에서도 버튼이 잘리지 않음
    if (btnGroup) {
        const root = document.documentElement;
        root.style.setProperty('--btn-group-h', btnGroup.offsetHeight + 'px');

        // 위에서 높이를 갱신했으니, 그 영향을 받은 game-wrapper의 실제 렌더 폭을 다시 읽어
        // 버튼 그룹 가로폭을 게임창과 정확히 동기화
        root.style.setProperty('--game-wrapper-w', wrapper.clientWidth + 'px');
    }
